...
function ENT:OnSpawn( PObj )
	--[[ basics ]]
	self:AddDriverSeat( Vector(-8,18,-20), Angle(0,-90,0) ) -- self:AddDriverSeat( Position,  Angle ) -- add a driver seat (max 1)
	-- Pod.ExitPos = Vector(0,0,100) -- change exit position
	-- Pod.HidePlayer = true -- should the player in this pod be invisible?

	local Pod = self:AddPassengerSeat( Vector(0,-18,-13), Angle(0,-90,0) ) -- add a passenger seat (no limit)
	local Pod = self:AddPassengerSeat( Vector(-35,18,-13), Angle(0,-90,0) )
	local Pod = self:AddPassengerSeat( Vector(-35,0,-13), Angle(0,-90,0) )
	local Pod = self:AddPassengerSeat( Vector(-35,-18,-13), Angle(0,-90,0) )
