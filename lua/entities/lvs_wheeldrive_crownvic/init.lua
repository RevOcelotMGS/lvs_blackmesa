...
function ENT:OnSpawn( PObj )
	--[[ basics ]]
	self:AddDriverSeat( Vector(-20,17,3), Angle(0,-90,0) ) -- self:AddDriverSeat( Position,  Angle ) -- add a driver seat (max 1)
	-- Pod.ExitPos = Vector(0,0,100) -- change exit position
	-- Pod.HidePlayer = true -- should the player in this pod be invisible?

	local Pod = self:AddPassengerSeat( Vector(-4,-20,10), Angle(0,-90,14.5) ) -- add a passenger seat (no limit)
	local Pod = self:AddPassengerSeat( Vector(-48,20,10), Angle(0,-90,14.5) )
	local Pod = self:AddPassengerSeat( Vector(-48,-20,10), Angle(0,-90,14.5) )
	...
	self:AddEngine( Vector(50,0,0) ) -- add a engine. This is used for sounds and effects and is required to get accurate RPM for the gauges.
	...
	-- example:
	local WheelModel = "models/props_vehicles/crownvic_tire.mdl"

	local WheelFrontLeft = self:AddWheel( { pos = Vector(59.1531,34.6314,3.89784), mdl = WheelModel, mdl_ang = Angle(0,180,0) } )
	local WheelFrontRight = self:AddWheel( { pos = Vector(59.1532,-34.0051,3.89784), mdl = WheelModel, mdl_ang = Angle(0,0,0) } )

	local WheelRearLeft = self:AddWheel( {pos = Vector(-71.0837,34.5435,3.89784), mdl = WheelModel, mdl_ang = Angle(0,180,0)} )
	local WheelRearRight = self:AddWheel( {pos = Vector(-71.0837,-34.0051,3.89784), mdl = WheelModel, mdl_ang = Angle(0,0,0)} )
